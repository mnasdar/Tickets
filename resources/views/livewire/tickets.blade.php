            ...
            @foreach ($tickets as $ticket)
                <tr>
                    ...
                    <td class="px-4 py-2 whitespace-nowrap">
                        ...
                        @else
                            <x-button-info data-tooltip-target="tooltip-info{{ $ticket->id }}"
                                wire:click="confirmTicketEdit({{ $ticket->id }})" wire:loading.attr="disabled">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="size-6 w-4 h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                            ...
                            <x-tooltip id='tooltip-info{{ $ticket->id }}'>Edit</x-tooltip>

                            <x-danger-button data-tooltip-target="tooltip-danger{{ $ticket->id }}"
                                wire:click="confirmTicketDeletion({{ $ticket->id }})" wire:loading.attr="disabled"
                                class="disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none
                                invalid:border-pink-500 invalid:text-pink-600
                                focus:invalid:border-pink-500 focus:invalid:ring-pink-500">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            ...
                            <x-tooltip id='tooltip-danger{{ $ticket->id }}'>Delete</x-tooltip>
                        @endif
                    </td>
                </tr>
            @endforeach
