<div
            ...
            @foreach ($data as $item)
                <tr>
                    ...
                    <td class="px-4 py-2 whitespace-nowrap">
                        <form action="{{ route('print.Ticket', $item->ticket_code) }}" target="_blank"
                            method="POST">
                            @if ($item->email)
                                <x-button-warning disabled data-tooltip-target="tooltip-email{{ $item->id }}" wire:loading.attr="disabled" class="disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none
                            ...
                            @else
                                <x-button-warning data-tooltip-target="tooltip-email{{ $item->id }}"
                                    wire:click="getEmail({{ $item->id }})" wire:loading.attr="disabled">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-6 w-4 h-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
                            ...
                            @endif
                            <x-tooltip id='tooltip-email{{ $item->id }}'>Send to Email</x-tooltip>
                            <x-button-info data-tooltip-target="tooltip-print{{ $item->id }}" type="submit"
                            ...
                            @csrf
                            <x-tooltip id='tooltip-print{{ $item->id }}'>Print Qr Code</x-tooltip>
                        </form>
                    </td>
                </tr>
            @endforeach
    ...
    <x-dialog-modal wire:model="getingEmail">
        <x-slot name="title">
            {{ __('Send to') }}
        </x-slot>
        ...
        <form wire:submit="sendMail">
            ...
            <x-slot name="footer">
                <x-secondary-button wire:click="$set('getingEmail',false)" wire:loading.attr="disabled">
                    {{ __('Cancel') }}
                </x-secondary-button>

                <x-button-save class="ms-3" wire:click="sendMail({{ $getingEmail }})"
                    wire:loading.attr="disabled">
                    {{ __('Save') }}
                    </x-danger-button>
