<div
    ...
    <div class="flex justify-between">
        <div class="p-2">
            <input wire:model.live.debounce.250ms="q" type="search" placeholder="" class="text-gray-700 shadow appearance-none border rounded w-full">
        </div>
        <div class="mr-2">
            <input type="checkbox" class="border rounded mr-2 leading-tight" wire:model.live="active" /> Active Only
        </div>
    </div>
            ...
            @foreach ($events as $event)
                <tr>
                    ...
                    <td class="px-4 py-2 whitespace-nowrap">
                        <x-info-button wire:click="confirmEventDeletion({{ $event->id }})"
    ...
    <x-dialog-modal wire:model.live="confirmingEventDeletion">
        ...
        <x-slot name="footer">
            ...
            <x-danger-button class="ms-3" wire:click="deleteEvent({{ $confirmingEventDeletion }})" wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-danger-button>
        </x-slot>
    </x-dialog-modal>
